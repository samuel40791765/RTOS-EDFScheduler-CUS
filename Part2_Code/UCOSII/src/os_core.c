            ...
            if (OSLockNesting == 0) {                      /* ... and not locked.                      */
                ...
                if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy */
                    ...
                    if(OSTCBCur->OSTCBPrio == OS_TASK_IDLE_PRIO){
                        printf("\t%d\t Completed\tTask(%d)(%d)\tTask(%d)(%d)\t\t\t\n",OSTimeGet(),OSTCBCur->task_id,OSTCBCur->task_times,OSTCBHighRdy->task_id,OSTCBHighRdy->task_times);
                    }
                    else{
                        if((OSTCBCur->OSTCBDly == 0)){
                             if(OSTCBCur->REMAINING_TIME > 0){
                                if(OSTCBCur->TASK_SHOULD_START_TIME < OSTimeGet()){
                                    preempting=1;
                                    printf("\t%d\t Preempted\tTask(%d)(%d)\tTask(%d)(%d)\t\t\t%d\n",OSTimeGet(),OSTCBCur->task_id,OSTCBCur->task_times,OSTCBHighRdy->task_id,OSTCBHighRdy->task_times,OSTCBCur->REMAINING_TIME);
                                }                             
                             }
                         }
                    }
                    //printf("%d %d %d %d\n",OSTimeGet(),OSTCBCur->task_id,OSTCBHighRdy->task_id,OSTCBHighRdy->OSTCBPrev->task_id);
...
void  OSTimeTick (void)
{
    ...
    if (OSRunning == OS_TRUE) {
        ...
        while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {     /* Go through all TCBs in TCB list              */
            OS_ENTER_CRITICAL();
            //printf("%d\n",OSTimeGet());
            if(ptcb->DEADLINE <= OSTimeGet() && ptcb->REMAINING_TIME>0 && ptcb->OSTCBPrio != OS_TASK_STAT_PRIO){
                printf("\t%d\t Miss Deadline\tTask(%d)(%d)\t-----------\n",OSTimeGet(),ptcb->task_id,ptcb->task_times);
                ptcb->task_times++;
                ptcb->DEADLINE = ptcb->DEADLINE + ptcb->period;
                ptcb->TASK_SHOULD_START_TIME = ptcb->period * ptcb->task_times; 
                OSTaskSuspend(ptcb->task_id);
            }
            if (ptcb->OSTCBDly != 0) {                     /* No, ed or waiting for event with TO     */
           ...
           if(ptcb->APERIODIC_INFO.aperiodictasknum!=0){
               int i=0;
                       ...
                       if(i>0){
                        if(ptcb->APERIODIC_INFO.arrivetime[i] < ptcb->APERIODIC_INFO.deadline[i-1]){
                           ...
                           ptcb->APERIODIC_INFO.actualarrivetime[i]=ptcb->APERIODIC_INFO.deadline[i-1];
                             ...
                             ptcb->DEADLINE = ptcb->APERIODIC_INFO.actualarrivetime[i] + ptcb->APERIODIC_INFO.compute[i]/ptcb->APERIODIC_INFO.server;
                            ...
                            OSTaskResume(ptcb->OSTCBPrio);
                            ptcb->REMAINING_TIME=ptcb->APERIODIC_INFO.compute[i];
                            //printf("%d %.2f",ptcb->APERIODIC_INFO.compute[i],ptcb->APERIODIC_INFO.server);
                            ptcb->TASK_SHOULD_START_TIME=ptcb->APERIODIC_INFO.arrivetime[i];
                            ptcb->DEADLINE = ptcb->APERIODIC_INFO.actualarrivetime[i] + ptcb->APERIODIC_INFO.compute[i]/ptcb->APERIODIC_INFO.server; 
                            ptcb->APERIODIC_INFO.deadline[i]=ptcb->DEADLINE;
                            printf("At Time %d, a new aperiodic job %d arrives and its deadline is updated to %d\n",OSTimeGet(),i,ptcb->DEADLINE);          
                       }
                   }
    ...
    if (OSIntNesting == 0) {                           /* Schedule only if all ISRs done and ...       */
        if (OSLockNesting == 0) {                      /* ... scheduler is not locked                  */
            ...
            if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy     */
                ...
                OSTCBHighRdy->OSTCBCtxSwCtr++;         /* Inc. # of context switches to this task      */
#endif
                 printf("\t%d\t Completed\tTask(%d)(%d)\tTask(%d)(%d)\t%d\n",OSTimeGet(),OSTCBCur->task_id,OSTCBCur->task_times,OSTCBHighRdy->task_id,OSTCBHighRdy->task_times,OSTCBCur->RESPONSE_TIME);                  
                 if(OSTCBCur->OSTCBPrio==0){
                    printf("Aperiodic Task %d's deadline is %d\n",OSTCBCur->task_times,OSTCBCur->DEADLINE);
                    if(OSTCBCur->REMAINING_TIME==0){
                        OSTCBCur->DEADLINE=999;//TRY TO GET DEADLINE OUT OF REACH
                    }
                 }
                 OSTCBCur->task_times++;
                //if( OSTCBCur->REMAINING_TIME!=0){
...
INT8U  OS_TCBInit (INT8U prio, OS_STK *ptos, OS_STK *pbos, INT16U id, INT32U stk_size, void *pext, INT16U opt)
{
    ...
    if (ptcb != (OS_TCB *)0) {
        ...
        ptcb->compute            = 0;
        ptcb->period             = 0;
        ptcb->APERIODIC_INFO.server           = 0;
        ptcb->APERIODIC_INFO.aperiodictasknum = 0;
        ptcb->APERIODIC_INFO.currenttask      = 0;
        int i=0;
        for(i=0;i<20;i++){
            ptcb->APERIODIC_INFO.arrivetime[i]=0;
            ptcb->APERIODIC_INFO.actualarrivetime[i]=0;
            ptcb->APERIODIC_INFO.compute[i]=0;
            ptcb->APERIODIC_INFO.deadline[i]=0;
        }
