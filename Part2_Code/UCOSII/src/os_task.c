...
INT8U  ChangePrio (INT8U oldprio, INT8U newprio)
{
#if (OS_EVENT_EN)
    OS_EVENT  *pevent;
...
INT8U  OSTaskCreate (void (*task)(void *p_arg), void *p_arg, OS_STK *ptos, INT8U prio, INT32U id ,INT32U ctime, INT32U ptime, double cpu, int aptasknum, int arrive[], int compute[])
{
    OS_STK    *psp;
    INT8U      err;
    ...
    if (OSTCBPrioTbl[prio] == (OS_TCB *)0) { /* Make sure task doesn't already exist at this priority  */
        ...
        OSTCBPrioTbl[prio]->task_id = id;
        OSTCBPrioTbl[prio]->compute = ctime; 
        OSTCBPrioTbl[prio]->period = ptime;
        if(prio!=0){
            OSTCBPrioTbl[prio]->DEADLINE = ptime; 
        }
        else{
            OSTCBPrioTbl[prio]->APERIODIC_INFO.server=cpu;
            OSTCBPrioTbl[prio]->APERIODIC_INFO.aperiodictasknum=aptasknum;
            OSTCBPrioTbl[prio]->DEADLINE = 20;
            int i=0;
            for(i=0;i<aptasknum;i++){
                OSTCBPrioTbl[prio]->APERIODIC_INFO.arrivetime[i]=arrive[i];
                OSTCBPrioTbl[prio]->APERIODIC_INFO.actualarrivetime[i]=arrive[i];
                OSTCBPrioTbl[prio]->APERIODIC_INFO.compute[i]=compute[i];
            }
        }
        if (err == OS_ERR_NONE) {
